...
if($comment_settings['active_plugin'] == 1) {
	?>
	<div id="fb-root"></div>
	<script async defer crossorigin="anonymous" src="https://connect.facebook.net/en_US/sdk.js#xfbml=1&version=v8.0&appId=<?php echo html_escape($comment_settings['facebook_app_id'], true); ?>&autoLogAppEvents=1" nonce="yOkzzrGk"></script>
	<?php
}
?>
		...
		<div class="container">
            <div class="selectionBoxMain">
                <div class="singlePost">
                    ...
                    <h3><a href="#pablo"><?php echo html_escape($post['title'], true) ?></a></h3>
                    <div class="singlePostDate"><i class="icon-calendar"></i> <?php if($post['datetime_updated'] != $post['datetime_added']){echo html_escape('Updated on: '.$post['datetime_updated'], true);}else{echo html_escape($post['datetime_added'], true);} ?></div>
                    <div class="singlePostDesc"><?php echo html_escape($post['description'], true) ?></div>
                    <hr>
						...
						if($comment_settings['active_plugin'] == 1) {
							...
							<script>
								var disqus_config = function () {
									...
									this.page.identifier = '<?php echo html_escape($permalink, true); ?>';
								};
								(function() {
									var d = document, s = d.createElement('script');
									s.src = 'https://<?php echo html_escape($comment_settings["disqus_short_name"], true); ?>.disqus.com/embed.js';
